...
model.JWT_ACCESS_TOKEN_EXPIRY_SECONDS = 40000; // 30 minutes
model.JWT_REFRESH_TOKEN_EXPIRY_SECONDS = 40000; // 14 days
...
model.generateToken = function (type, req, callback) {
  ...
  if (type === "accessToken") {
    ...
    exp = exp + model.JWT_ACCESS_TOKEN_EXPIRY_SECONDS;
  } else {
    secret = JWT_SECRET_FOR_REFRESH_TOKEN;
    exp = exp + model.JWT_REFRESH_TOKEN_EXPIRY_SECONDS;
  }
  payload.exp = exp;
...
model.getAccessToken = function (bearerToken, callback) {
  return JWT.verify(bearerToken, JWT_SECRET_FOR_ACCESS_TOKEN, function (
    err,
    decoded
  ) {
    if (err) {
...
model.saveAccessToken = function (
  accessToken,
  clientId,
  expires,
  userId,
...
model.getRefreshToken = function (bearerToken, callback) {
  return JWT.verify(bearerToken, JWT_SECRET_FOR_REFRESH_TOKEN, function (
    err,
    decoded
  ) {
    if (err) {
...
// As we're using JWT there's no need to store the token after it's generated
model.saveRefreshToken = function (
  refreshToken,
  clientId,
  expires,
  userId,
